...
sp_500_daily_returns_tbl <- sp_500_prices_tbl %>% 
    # Select columns and period to focus on
    select(symbol, date, adjusted) %>% 
    filter(date >= ymd("2018-01-01")) %>% 
    
    # Create lag adjusted price by 1 day
    group_by(symbol) %>% 
    mutate(adjusted_lag_1 = lag(adjusted)) %>% 
    ungroup() %>% 
    drop_na() %>% 
    
    # Compute return and pct_return
    mutate(return = adjusted - adjusted_lag_1) %>% 
    mutate(pct_return = return / adjusted_lag_1) %>% 
...
```{r}
sp_500_daily_returns_tbl_2 <- read_rds("challenges/week_6_data/sp_500_daily_returns_tbl.rds")
...
k_means_mapped_tbl %>% 
    ...
    labs(title = "Scree Plot", 
         subtitle = "Scree Plot becomes linear between 5 and 10 centers")
```
...
umap_kmeans_results_tbl <- k_means_obj %>% 
    ...
    left_join(
        sp_500_index_tbl %>% as_tibble() %>% select(symbol, company, sector)
    )
