...
(value / 1e6) %>% scales::number(prefix = "R$ ", suffix = "M")
...
bike_orderlines_calnames_tbl <- bike_orderlines_tbl %>% 
    rename_at(.vars = vars(model:frame_material), 
              .funs = ~ str_c("prod_", .)) %>% 
    rename_at(vars(bikeshop_name:state), ~ str_c("cust_", .))

bike_orderlines_calnames_tbl %>% 
    select(contains("cust"), total_price)
...
bikes_tbl %>% 
    ...
    # Get tier feature
    mutate(model_tier = model %>% str_replace(model_base, "") %>% str_trim(side = "left")) %>% 
    
    # Remove uunecessary columns
    select(-matches("[0-9]")) %>% 
