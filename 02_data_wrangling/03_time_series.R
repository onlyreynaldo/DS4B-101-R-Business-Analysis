...
"2011-01-01" %>% ymd() %>% month(label = TRUE)
...
now() # datetime class: "POSIXct" "POSIXt"

today() # date class: "Date"
...
today() + dyears(4) # Duration - always start with a "d"
...
i / ddays(1) # interval / days = how many days in intervals
...
bike_sales_y_tbl %>% calculate_pct_diff()
...
bike_sales_m_tbl %>% 
    ...
    mutate(
        diff = sales - sales_jan, 
        pct_diff = diff / sales_jan, 
        pct_diff_chr = scales::percent(pct_diff)
    ) %>% 
...
bike_sales_m_tbl %>% 
    mutate(roll_mean_3 = zoo::rollmean(sales, k = 3, 
                                       na.pad = TRUE, 
                                       align = "right", 
                                       fill = NA)
           ) %>% 
    mutate(roll_mean_6 = zoo::rollmean(sales, k = 6, 
                                       na.pad = TRUE, 
                                       align = "right", 
                                       fill = NA)
           )
