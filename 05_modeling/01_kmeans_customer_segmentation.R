...
customer_trends_tbl <- bike_orderlines_tbl %>% 
    ...
    summarise(quantity_purchase = sum(quantity)) %>% 
    ungroup() %>% 
    
    # Proportion
    group_by(bikeshop_name) %>% 
    mutate(prop_of_total = quantity_purchase / sum(quantity_purchase)) %>% 
    ungroup()
...
# Returns the centers information for the kmeans model
broom::tidy(kmeans_obj) %>% glimpse()

# Returns overall summary metrics for the model
broom::glance(kmeans_obj) 

# Add column to the original data that was modeled with the predicted cluster
...
# Mapping the funtion to many elements
...
kmeans_mapped_tbl %>% 
    ...
    labs(
        ...
        caption = "Based on the Skree Plot, we select 4 clusters to segment the customer base."
    )
